...
class CFilter {
	...
	public:
		...
		float operator()(float newAngle, float newRate) {
			angleC = a * (angleC + newRate * dT) + (1 - a) * newAngle;
			return angleC;
		}

		/* Wird aktuell nicht benutzt */
		float filter2(float newAngle, float newRate) {
			const float k = 10;
			float x1 = (newAngle - angleC) * k * k;
			y1 += dT * x1;
			float x2 = y1 + (newAngle - angleC) * 2 * k + newRate;
			angleC += dT * x2;
